...
|----|----|----|
| `name` | single character | Unique identifier; becomes the `source` column in all output and the registry key |
...
| `fetch_flows_fn` | `function(station_number, start_date, end_date)` or `NULL` | Returns a tibble matching the realtime schema; `NULL` if sub-daily flow data is not available |
| `fetch_daily_flows_fn` | `function(station_number, start_date, end_date)` or `NULL` | Returns a tibble matching the daily schema; `NULL` if daily flow data is not available |
| `fetch_levels_fn` | `function(station_number, start_date, end_date)` or `NULL` | Returns a tibble matching the realtime schema with `parameter = "level"`; `NULL` if sub-daily level data is not available |
| `fetch_daily_levels_fn` | `function(station_number, start_date, end_date)` or `NULL` | Returns a tibble matching the daily schema with `parameter = "level"`; `NULL` if daily level data is not available |
...
| Column | Type | Notes |
|----|----|----|
| `station_number` | chr | As provided by the caller |
| `datetime` | POSIXct UTC | Sub-daily observations |
| `value` | dbl |  |
| `parameter` | chr | `"flow"` or `"level"` |
| `units` | chr | Canonical form after normalization (e.g. `"m3/s"`, `"m"`) |
| `source` | chr | Must equal the adapter name |
...
- Only flow data is available (`parameter = "flow"`); no water level.
...
.myprov_fetch_flows <- function(station_number, start_date, end_date) {
  resp <- httr2::request(.MYPROV_URL) |>
    httr2::req_url_query(
      endpoint = "timeseries",
  ...
  tibble::tibble(
    ...
    datetime       = as.POSIXct(resp$timestamp, tz = "UTC"),
...
.myprov_mock_fetch_flows <- function(station_number, start_date, end_date) {
...
It also normalises the `units` column: common variants such as `"m³/s"`,
