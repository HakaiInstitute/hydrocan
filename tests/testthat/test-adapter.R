test_that("new_hydrocan_adapter rejects non-string name", {
  expect_error(new_hydrocan_adapter(123L, "desc", identity, fetch_flows_fn = identity), "name")
  expect_error(new_hydrocan_adapter(c("a", "b"), "desc", identity, fetch_flows_fn = identity), "name")
  expect_error(new_hydrocan_adapter("", "desc", identity, fetch_flows_fn = identity), "name")
})

test_that("new_hydrocan_adapter rejects non-string description", {
  expect_error(new_hydrocan_adapter("ok", 123L, identity, fetch_flows_fn = identity), "description")
})
...
test_that("new_hydrocan_adapter rejects non-function arguments", {
  ...
  expect_error(
    new_hydrocan_adapter("ok", "desc", identity, fetch_daily_flows_fn = "not_fn"),
...
test_that("new_hydrocan_adapter accepts only fetch_flows_fn", {
  a <- new_hydrocan_adapter("flows_only", "desc", identity, fetch_flows_fn = identity)
...
test_that("new_hydrocan_adapter accepts only fetch_daily_flows_fn", {
  a <- new_hydrocan_adapter("daily_only", "desc", identity, fetch_daily_flows_fn = identity)
  expect_s3_class(a, "hydrocan_adapter")
  expect_null(a$fetch_flows_fn)
})

test_that("new_hydrocan_adapter returns a correctly structured object", {
  a <- new_hydrocan_adapter("mytest", "A test adapter.", identity, fetch_flows_fn = identity)
  expect_s3_class(a, "hydrocan_adapter")
...
test_that("register_hydrocan_adapter stores adapter in registry", {
  a <- new_hydrocan_adapter("reg_test", "desc", function() character(), fetch_flows_fn = function(...) NULL)
  register_hydrocan_adapter(a)
...
test_that("hc_list_sources returns the correct schema", {
  ...
  expect_named(result, c("name", "description", "has_flows", "has_daily_flows", "has_stations"))
