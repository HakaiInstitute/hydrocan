...
Passing `source = "adaptername"` skips steps 1-3 entirely.
...
.myprov_fetch_flows <- function(station_number, start_date, end_date) {
  ...
  tibble::tibble(
    station_number = resp$station_id,
...
Create `tests/testthat/test-myprov.R` with integration tests guarded by
`skip_if_offline()`:

```r
test_that("myprov adapter returns valid schema", {
  skip_if_offline()
...
The mock adapter in `tests/testthat/helper-mocks.R` shows how to write offline
unit tests that exercise the router and schema validation logic without making
any HTTP requests. The `local_register_adapter()` helper registers an adapter
for the duration of a single test and restores the registry state on exit.

### What the schema validator will catch

`validate_hydrocan_schema()` is called automatically after every public API
call. It will stop with a clear message if:

- Any required column is missing from the returned tibble.
- The `approval` column contains a non-`NA` value other than `"provisional"`,
